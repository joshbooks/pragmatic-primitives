...
object WorkStealingHelper : BaseHelper() {
    ...
    sealed class NextIndexReturnValue(val nextIndex: Int) {
        ...
        class WrapToNextThread(nextIndex: Int, newEffectiveThreadNumber: Int) : NextIndexReturnValue(nextIndex)
    }
    ...
    fun nextIndex(
        prevIndex: Int,
        effectiveNumThreadsHelping: Int,
        effectiveHelperNumber: Int,
        numRecords: Int
    ): NextIndexReturnValue {
        // ok, so there are basically two sensible ways I can think to do this, I propose to eventually make these
        // into two subclasses of this workstealing class and benchmark them to decide which makes more sense,
        // but will just describe them here for now and go with the way that seems cooler for now. I think for all of
        // them it makes sense to have threads that are greater than the closest power of two less than numThreadsHelping
        // just duplicate work so there will some bit twiddling to figure out the appropriate effective thread number
        // no matter what.
        ...
        // index (with some additional jiggery pokers needed to handle changes in numThreadsHelping and effectiveHelperNumber)
        // Second is a chunk based approach, in which we treat the list of records as a set of blocks that we split in
        // half for every doubling of the number of effective threads. So we start with one block and one thread
        // then two blocks for two threads, then 4 for 4, etc. This has the benefit of maximizing locality because
        // each thread is just reading sequentially through a list from some starting point, so we make efficient
        // use of the cache. This strategy also has the benefit of repeating less work, since elements at the end
        // are the ones least likely to have been handled by the existing thread.
        // I think the second approach is cooler, so let's go with that for now and see how it works out.
        ...
        val myBucketEndPoint =
            min(
                (numRecords / effectiveNumThreadsHelping) * (effectiveHelperNumber + 1),
                numRecords
            )

        // hate to do branching in such a hot path, seems like there's probably some bit-twiddly magic we could
        // avail ourselves of here, but doing ugly if thens for now to express intent
        if (prevIndex < myBucketStartingPoint) {
            // definitely hit this case on the first go, don't think there are any other cases though since this
            ...
            return NextIndexReturnValue.WrapToNextThread(
                startingPointForEffectiveHelperNumber(
                    newEffectiveHelperNumber
                ),
                newEffectiveHelperNumber
            )
        }
    ...
    override suspend fun help(scxRecord: DataRecord.ScxRecord): Boolean {
        ...
        // after completing our work we don't have a great way to tell that we're finished doing other thread's work
        ...
        // it is helpful to note that threadsHelping is effectively monotonically increasing so long as the
        // cooperative part of the scx is still ongoing. Any threads that crash will of course stop contributing
        ...
        try {
            ...
            while (true) {
                ...
                if (!r.info.compareAndSet(rinfo, scxRecord)) {
                    if (r.info.get() !== scxRecord) {
                        ...
                        if (scxRecord.allFrozen) {
                            ...
                            scxRecord.state =
                                DataRecord.ScxRecord.ScxState.ABORTED
                            return false
                        }
                    }
                }
                ...
                // if we were done according to the count we were using to calculate the index we just finished then
            ...
            scxRecord.recordToModify?.mutableFields
